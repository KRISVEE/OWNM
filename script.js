...
let appState = JSON.parse(localStorage.getItem(dateKey)) || { exercises: {} };
// --- STREAK MANAGEMENT ---
const streakKey = 'aura_streak_data';
let streakData = JSON.parse(localStorage.getItem(streakKey)) || { count: 0, lastCompletedDate: null };
...
    
    if (diffDays === 0) isStreakValid = true; // Already done today
    else if (diffDays === 1) isStreakValid = true; // Did it yesterday
    else if (diffDays === 2 && currentDayOfWeek === 1) isStreakValid = true; // Sat -> Mon (Skipped Sunday)

    if (!isStreakValid) {
        streakData.count = 0; // Reset streak
...
    if (workout.exercises.length === 0) return; // Rest day
    
    let total = 0;
    let completed = 0;
    
...
    // If fully completed and not already logged today
...
        }
    } else {
        // Handle edge case: User unchecks a box after completing
        if (streakData.lastCompletedDate === todayStr) {
            streakData.count = Math.max(0, streakData.count - 1);
            
            // Revert date to yesterday to keep previous streak alive
...
    renderFlowSetData();
    
    // Switch views instantly and scroll to top for native feel
...
    renderFlowSetData();
    checkWorkoutCompletion(); // <-- ADDED THIS LINE TO UPDATE STREAK
